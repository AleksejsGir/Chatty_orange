...
class ChatWithAIView(View):
    ...
    def handle_natural_language_query(self, user_input: str, user_info: dict) -> str:
        ...
        user_search_patterns = [
            'найди пользователя', 'найти пользователя', 'ищи пользователя', 'искать пользователя',
            'найди юзера', 'найти юзера', 'кто такой', 'покажи пользователя',
            'пользователь', 'профиль', 'найди @', 'юзер'
        ]

        if any(pattern in lower_input for pattern in user_search_patterns):
            username = self.extract_username(user_input)
            if username:
                logger.info(f"Extracted username: '{username}' from query: '{user_input}'")
                return find_user_by_username(username, user_info)
            else:
                return "🔍 Укажите имя пользователя для поиска. Например: 'Найди пользователя Orange' или 'Пользователь admin'"

        # В функции handle_natural_language_query замените секцию поиска постов на эту:
        ...
        if any(pattern in lower_input for pattern in post_search_patterns):
            ...
            if any(phrase in lower_input for phrase in user_posts_patterns):
